...
function Article({ userId }) {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_REQUEST_URL}/api/article/view/${articleId}`)
      .then((response) => {
  ...
  const onDeleteClick = async () => {
    ...
    const response = await axios.post(
      `${process.env.REACT_APP_REQUEST_URL}/api/article/delete`,
      {
        articleId,
      }
    );
  ...
  const onVoteClick = async (e) => {
    ...
    const response = await axios.post(
      `${process.env.REACT_APP_REQUEST_URL}/api/article/vote`,
      formData
    );
    errorHandler(response.data.data);
    setArticle({
    ...
    <div className="container m-auto is-max-desktop">
      <div className="block">
        ...
        <div className="article-item level">
          <div className="article-writer level-left mb-1">
            {article.userName}
          </div>
      ...
      <div className="buttons is-centered">
        <button
          className="button is-info is-light is-size-5 has-text-weight-bold"
          name="u"
          onClick={onVoteClick}
        >
          추천 {article.upVote}
        ...
        <button
          className="button is-danger is-light is-size-5 has-text-weight-bold"
          name="d"
          onClick={onVoteClick}
        >
