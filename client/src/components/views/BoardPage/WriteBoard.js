...
function Write() {
  ...
  const onClick = async () => {
    if (!data.subject.trim() || !data.text.trim())
      return alert("제목이나 내용을 입력해 주세요.");
    if (state) {
      const response = await axios.post(
        `${process.env.REACT_APP_REQUEST_URL}/api/article/modify`,
        {
          ...data,
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_REQUEST_URL}/api/article/write`,
        data
      );
      errorHandler(response.data.data);
    }
  ...
  const uploadImage = async (blob) => {
    ...
    const response = await axios.post(
      `${process.env.REACT_APP_REQUEST_URL}/api/file/upload`,
      formData,
      {
        headers: {
    ...
    return (
      `${process.env.REACT_APP_REQUEST_URL}/api/file/` + response.data.data
    );
  };
